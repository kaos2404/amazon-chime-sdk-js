...
describe('DefaultAudioVideoController', () => {
  ...
  describe('reconnect for no attendee presence', () => {
    ...
    it('attempts to get user media in Firefox during reconnect for no ice candidates', function(done) {
      setUserAgent(
      ...
      delay().then(async () => {
        ...
        webSocketAdapter.send(makeIndexFrame());
        await delay(300);
        // @ts-ignore "gathering" state
        audioVideoController.rtcPeerConnection.dispatchEvent(makeICEEvent(null));
        // @ts-ignore "complete" state to trigger the "no ice candidate" error
        audioVideoController.rtcPeerConnection.dispatchEvent(makeICEEvent(null));
        await delay();
        webSocketAdapter.send(makeSubscribeAckFrame());
        await delay();
